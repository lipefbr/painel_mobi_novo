    ...
    <style>
        ...
        @if(!empty(Session::get('user')->language) && Session::get('user')->language=='ar')
        .arabic_rtl{
...
<div class="arabic_rtl" style="width: 80%; margin: auto;">
    <h1>Gestor de Traduções</h1>
    <p>Aviso, as traduções não são visíveis até que sejam exportadas de volta para o arquivo app / lang, usando o botão Publicar.</p>
    <div class="alert alert-success success-import" style="display:none;">
        <p>Importação concluída, processada<strong class="counter">N</strong> Itens! Recarregue esta página para atualizar os grupos!</p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p>Concluído a pesquisa de traduções, encontrado<strong class="counter">N</strong> items!</p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p>Publicação concluída das traduções para o grupo'<?= $group ?>'!</p>
    </div>
    ...
    <p>
        ...
        <form class="form-inline form-import" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
            ...
            <select name="replace" class="form-control">
                <option value="0">Anexar novas traduções</option>
                <option value="1">Substituir traduções existentes</option>
            </select>
            <button type="submit" class="btn btn-success"  data-disable-with="Loading..">Importar Grupos</button>
        </form>
        <form class="form-inline form-find" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Tem certeza de que deseja verificar a pasta do aplicativo? Todas as chaves de tradução encontradas serão adicionadas ao banco de dados.">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <p></p>
            <button type="submit" class="btn btn-info" data-disable-with="Searching.." >Encontre traduções em arquivos</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Tem certeza de que deseja publicar o grupo de traduções'<?= $group ?>? Isso substituirá os arquivos de idioma existentes.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-info" data-disable-with="Publishing.." >Publicar Traduções</button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default">Voltar</a>
            </form>
        <?php endif; ?>
    </p>
    <form role="form">
    ...
    <?php else: ?>
        <p>Escolha um grupo para exibir as traduções do grupo. Se nenhum grupo estiver visível, verifique se você executou as migrações e importou as traduções.</p>
