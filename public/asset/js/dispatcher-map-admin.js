...
function createRideInitialize() {
    ...
    function updateEstimated(){
        $.ajax({
           ...
           { 
               $("#estimated").text(data.estimated_fare.toLocaleString("pt-BR", { style: "currency" , currency:"BRL"}));
...
function assignProviderShow(providers, trip) {
    ...
    bounds.extend({lat: parseFloat(trip.s_latitude), lng: parseFloat(trip.d_longitude)});
    ...
    providers.forEach(function(provider) {
        ...
        var content = "<p>Nome : "+provider.first_name+" "+provider.last_name+"</p>"+
                "<p>Telefone : "+provider.mobile+"</p>"+
                "<p>Avaliações : "+provider.rating+"</p>"+
                "<p>Serviço : "+provider.service.service_type.name+"</p>"+
                "<p>Veículo  : "+provider.service.service_model+"</p>"+
                "<a href='/admin/dispatcher/trips/"+trip.id+'/'+provider.id+"' class='btn btn-success'>Atribuir ao Motorista</a>";
