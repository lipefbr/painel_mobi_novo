...
class TokenController extends Controller
{
	...
	public function register(Request $request)
	{
        ...
        if(Provider::withoutGlobalScopes()->where('email', $request->email)->first()) {
        	if ($request->ajax() || $request->wantsJson()) {
				return response()->json(["Motorista já registrado! Realize seu login!"], 500);
			}
        }

        $mobile = str_replace(['(', ')', '-', ' '], '', $request->mobile);
        
        if(Provider::withoutGlobalScopes()->where('mobile', $mobile)->count() > 0) {
        	if ($request->ajax() || $request->wantsJson()) {
				return response()->json(['error' => "Telefone já cadastrado, tente outro!"], 500);
			}
        }
		...
		try{
            ...
            $service = ServiceType::withoutGlobalScopes()->findOrFail($request->service_type);

            $Provider['fleet'] = $service->fleet_id;

			$Provider = Provider::withoutGlobalScopes()->create($Provider);
...
//			}
	        ...
	        if(setting('referral', config('constants.referral', 0)) == 'on') {
	            if($request->referral_code != null){
...
//				}
			...
			if($AuthUser){
				...
				return response()->json([
							...
							'currency' => setting('currency', config('constants.currency', '$')),
							'measurement' => setting('distance', config('constants.distance', 'Kms')),
							'sos' => setting('sos_number', config('constants.sos_number', '911'))
						]);
			}else{
...
//				}
	...
	public function refresh_token(Request $request)
	{
		...
		$Provider->currency = setting('currency', config('constants.currency', '$'));
		$Provider->sos = setting('sos_number', config('constants.sos_number', '911'));
		$Provider->measurement = setting('distance', config('constants.distance', 'Kms'));
	...
	public function settings(Request $request)
	{
		...
		if($request->has('city_id')){
            $serviceType = ServiceType::select('id', 'name', 'service_type_vehicle')->where('status', 1)
            ->whereHas('fleet', function ($query) use($request){
                $query->where('city_id',$request->city_id);
            })->get();
        }else{
            $serviceType = ServiceType::select('id', 'name', 'service_type_vehicle')->where('status', 1)->get();
        }

        foreach ($serviceType as $k =>  $serviceT) {
        	$serviceT['name'] = $serviceT['name'] .' - ' . config('utils.service_type_vehicle')[$serviceT['service_type_vehicle']];
        }

		$settings = [
			'serviceTypes' => $serviceType,
			'referral' => [
				'referral' => setting('referral', config('constants.referral', 0)),
				'count' => setting('referral_count', config('constants.referral_count', 0)),
				'amount' => setting('referral_amount', config('constants.referral_amount', 0)),
