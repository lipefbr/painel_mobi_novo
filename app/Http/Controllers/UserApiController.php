...
class UserApiController extends Controller
{
    ...
    public function signup(Request $request)
    {
        ...
        //User Already Exists
        if ($email_case != null) {
            return response()->json(['E-mail já cadastrado!'], 422);
        }
        ...
        if ($registeredEmail != null && $registeredMobile != null) {
            ...
            if ($registeredEmail != null)
                ...
                return response()->json(['Telefone já cadastrado!'], 422);
        } else {
            if ($registeredEmail != null)
        ...
        if ($currentUser == null) {
            ...
            $User['currency'] = setting('currency', config('constants.currency'));
            $User['sos'] = setting('sos_number', config('constants.sos_number', '911'));
            $User['app_contact'] =  str_replace(['(', ')', '-', ' '], '', setting('app_contact', config('constants.app_contact', '5777')));
            $User['measurement'] = setting('distance', config('constants.distance', 'Kms'));
        } else {
            $User = $currentUser;
        ...
        *
        if (setting('send_email', config('constants.send_email', 0)) == 'on') {
            Helper::site_registermail($User);
        }
        */

        //check user referrals
        if (setting('referral', config('constants.referral')) == 'on') {
            if ($request->referral_code != null) {
    ...
    public function details(Request $request)
    {
        ...
        try {
            ...
            if ($user = User::find(Auth::user()->id)) {
                ...
                $user->cash = setting('cash', config('constants.cash')) == 'on' ? 1 : 0;
                
                //TODO ALLAN - Alterações débito na máquina e voucher
                $user->debit_machine = setting('debit_machine',  config('constants.debit_machine')) == 'on' ? 1 : 0;
                $user->voucher = setting('voucher', config('constants.voucher')) == 'on' ? 1 : 0;
                
                $user->card = setting('card', config('constants.card')) == 'on' ? 1 : 0;

                $user->stripe_secret_key = setting('stripe_secret_key', config('constants.stripe_secret_key'));
                $user->stripe_publishable_key = setting('stripe_publishable_key', config('constants.stripe_publishable_key'));
                $user->stripe_currency = setting('stripe_currency', config('constants.stripe_currency'));


                $user->referral_count = setting('referral_count', config('constants.referral_count', '0'));
                $user->referral_amount = setting('referral_amount', config('constants.referral_amount', '0'));
                ...
                $user->referral_total_text = "<p style='font-size:16px; color: #000; $align'>" 
                                            . trans('api.user.referral_amount') 
                                            . ": " 
                                            . (new ReferralResource)->get_referral('user', Auth::user()->id)[0]->total_amount 
                                            . "<br>" . trans('api.user.referral_count') 
                                            . ": " . (new ReferralResource)->get_referral('user', Auth::user()->id)[0]->total_count 
                                            . "</p>";

                $user->ride_otp = (int)setting('ride_otp', config('constants.ride_otp')) == 'on' ? 1 : 0;
                $user->ride_toll = (int)setting('ride_toll', config('constants.ride_toll')) == 'on' ? 1 : 0;
                return $user;
            } else {
                return response()->json(['error' => trans('api.user.user_not_found')], 422);
            }
    ...
    public function update_profile(Request $request)
    {
        ...
        try {
            ...
            $user->referral_total_text = "<p style='font-size:16px; color: #000; $align'>" 
                                        ...
                                        . "<br>" . trans('api.user.referral_count') 
                                        . ": " 
                                        . (new ReferralResource)->get_referral('user', Auth::user()->id)[0]->total_count . "</p>";
    ...
    public function services(Request $request)
    {
        ...
        */
        ...
        if ($serviceList->count() > 0 && $Providers->count() > 0) {
            return $serviceList;
        } else {
    ...
    {
        //\Log::info('USER_SEND_REQUEST ===> ' . json_encode($request->all()));
        ...
        $Providers = Provider::with('service')
            ...
            //Busca apenas provider com maquina de cartao
            if ($request->payment_mode == "DEBIT_MACHINE") {
                $Providers = $Providers->where('providers.debit_machine', 1);
            }

            //Caso o pagamento nao for em cartao, busque apenas provider sem divida no limite
            if ($request->payment_mode != 'CARD') {
                 $Providers = $Providers->where('wallet_balance', '>', setting('minimum_negative_balance', config('constants.minimum_negative_balance')));
            }

            $Providers = $Providers->orderBy('distance', 'asc')->get();


        if (count($Providers) == 0) {
            if ($request->ajax()) {
                if($request->payment_mode == "DEBIT_MACHINE"){
                     return response()->json(['error' => "Nenhum motorista com MAQUINA DE CARTÃO encontrado! Selecione outro meio de pagamento e tente novamente!"], 422);
                }
        ...
        try {
            ...
            $UserRequest->device_type = Auth::user()->device_type;


            if ((setting('manual_request', config('constants.manual_request', 0)) != 'on') && ( setting('broadcast_request', config('constants.broadcast_request', 0)) != 'on') ) {
            ...
            if ($Providers->count() <= setting('surge_trigger', config('constants.surge_trigger')) && $Providers->count() > 0) {
                $UserRequest->surge = 1;
            }
    ...
    public function extend_trip(Request $request)
    {
        ...
        try {
            ...
            $details = "https://maps.googleapis.com/maps/api/directions/json?origin=" . $UserRequest->s_latitude . "," . $UserRequest->s_longitude . "&destination=" . $request->latitude . "," . $request->longitude . "&mode=driving&key=" . setting('map_key', config('constants.map_key'));
    ...
    public function request_status_check()
    {
        ...
        try {
            ...
            if (!empty($UserRequests)) {
                $UserRequests[0]['ride_otp'] = (int)setting('ride_otp',config('constants.ride_otp', 0)) == 'on' ? 1 : 0;
                $UserRequests[0]['ride_toll'] = (int)setting('ride_toll', config('constants.ride_toll', 0)) == 'on' ? 1 : 0;
            ...
            $type = setting('broadcast_request', config('constants.broadcast_request', 0));
            ...
            return response()->json(['data' => $UserRequests,
                'sos' => setting('sos_number', config('constants.sos_number', '190')),
                'cash' => (int) setting('cash', config('constants.cash')) == 'on' ? 1 : 0,
                
                //TODO ALLAN - Alterações débito na máquina e voucher
                'debit_machine' => setting('debit_machine', config('constants.debit_machine')) == 'on' ? 1 : 0,
                'voucher' => (int) setting('voucher', config('constants.voucher')) == 'on' ? 1 : 0,
                
                'card' => setting('card', config('constants.card')) == 'on' ? 1 : 0,
                'currency' => setting('currency', config('constants.currency', 'R$')),
                
                
                'stripe_secret_key' => setting('stripe_secret_key', config('constants.stripe_secret_key')),
                'stripe_publishable_key' => setting('stripe_publishable_key', config('constants.stripe_publishable_key')),
                'stripe_currency' => setting('stripe_currency', config('constants.stripe_currency')),
...
//                }
    ...
    //Enabled virtual change ride
    public function virtualChangeRide($id)
    {
        try {

            $UserRequest = UserRequests::findOrFail($id);

            if($UserRequest->virtual_change_allowed) {
                $UserRequest->update(['virtual_change_allowed' => false]);

                return response()->json(['message' => "Troco virtual desabilitado!"], 201);
            } else {
                $UserRequest->update(['virtual_change_allowed' => true]);

                return response()->json(['message' => "Troco virtual habilitado!"], 200);
            }

        } catch (Exception $e) {
             return response()->json(['error' => "Falha ao habilitar troco virtual!"], 500);
        }
    ...
    public function estimated_fare(Request $request)
    {
        ...
        try {
            ...
            //Log::info('RESPONSE ESTIMATE FARE ===>' .  json_encode($responsedata));
    ...
    public function trip_details(Request $request)
    {
        ...
        try {
            ...
            if (!empty($UserRequests)) {
                ...
                $UserRequests[0]->contact_number = setting('contact_number', config('constants.contact_number', ''));
                $UserRequests[0]->contact_email = setting('contact_email', config('constants.contact_email', ''));
            }
    ...
    public function CheckVersion(Request $request)
    {
        ...
        try {
            ...
            if ($sender == 'user') {
                if ($device_type == 'ios') {
                    $curversion = setting('version_ios_user', config('constants.version_ios_user'));
                    if ($curversion == $version) {
                        return response()->json(['force_update' => false]);
                    } elseif ($curversion > $version) {
                        return response()->json(['force_update' => true, 'url' =>  setting('store_link_ios_user',config('constants.store_link_ios_user'))]);
                    } else {
                        return response()->json(['force_update' => false]);
                    }
                } else {
                    $curversion =  setting('version_android_user', config('constants.version_android_user'));
                    if ($curversion == $version) {
                        return response()->json(['force_update' => false]);
                    } elseif ($curversion > $version) {
                        return response()->json(['force_update' => true, 'url' =>  setting('store_link_android_user', config('constants.store_link_android_user'))]);
                    } else {
                        return response()->json(['force_update' => false]);
                    }
                }
                ...
                if ($device_type == 'ios') {
                    $curversion =  setting('version_ios_provider', config('constants.version_ios_provider'));
                    if ($curversion == $version) {
                        return response()->json(['force_update' => false]);
                    } elseif ($curversion > $version) {
                        return response()->json(['force_update' => true, 'url' =>  setting('store_link_ios_provider', config('constants.store_link_ios_provider'))]);
                    } else {
                        return response()->json(['force_update' => false]);
                    }
                } else {
                    $curversion =  setting('version_android_provider', config('constants.version_android_provider'));
                    if ($curversion == $version) {
                        return response()->json(['force_update' => false]);
                    } elseif ($curversion > $version) {
                        return response()->json(['force_update' => true, 'url' =>  setting('store_link_android_provider', config('constants.store_link_android_provider'))]);
                    } else {
                        return response()->json(['force_update' => false]);
                    }
                }
    ...
    public function settings(Request $request)
    {
        ...
        $settings = [
            ...
            'referral' => [
                'referral' =>  setting('referral', 0) == 'on' ? 1 : 0,
                'count' => setting('referral_count', 0),
                'amount' =>  setting('referral_amount', 0),
                'ride_otp' => setting('ride_otp', 0) == 'on' ? 1 : 0,
                'ride_toll' => setting('ride_toll', 0) == 'on' ? 1 : 0,
            ]
        ];
