    ...
    public function calculateFare($request, $cflag=0){
        ...
        try{
            ...
            if(!empty($location['errors'])){
                ...
                if( setting('distance', config('constants.distance','Kms'))=='Kms')
                ...
                $tax_percentage =  setting('tax_percentage', config('constants.tax_percentage'));
                $commission_percentage = setting('commission_percentage', config('constants.commission_percentage'));
                
                \Log::info('COMISSAOO ====> ' . $commission_percentage);
                \Log::info('SERVICE TYPE ====> ' . json_encode($request['service_type']));
                \Log::info('REQUEST ====> ' . json_encode($request));
                
                $surge_trigger =  setting('surge_trigger', config('constants.surge_trigger'));
                ...
                if($cflag!=0){
                    ...
                    if($surge_trigger>0){
                        ...
                        $Providers = Provider::whereIn('id', $ActiveProviders)
                            ...
                            ->whereRaw("(1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")->get();

                        $surge = 0;

                        if($Providers->count() <= setting('surge_trigger', config('constants.surge_trigger')) && $Providers->count() > 0){
                            $surge_price = $this->applyPercentage($total, setting('surge_percentage', config('constants.surge_percentage')));
                            $total += $surge_price;
                            $surge = 1;
                        }
                    ...
                    $surge_percentage = 1+(setting('surge_percentage', config('constants.surge_percentage'))/100)."X";
