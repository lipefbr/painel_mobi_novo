...
class Helper
{
	...
	public static function generate_booking_id() {
		return setting('booking_prefix', config('constants.booking_prefix')).mt_rand(100000, 999999);
	}
	...
	public static function site_sendmail($user){
		...
		Mail::send('emails.invoice', ['Email' => $user], function ($mail) use ($user,$site_details) {
			...
			$mail->to($user->user->email, $user->user->first_name.' '.$user->user->last_name)->subject('Fatura');
		});
